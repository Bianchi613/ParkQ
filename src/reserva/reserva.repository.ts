...
export class ReservaRepository {
  ...
  async findReservaById(id: number): Promise<Reserva> {
    return await this.reservaModel.findByPk(id, { include: ['usuario', 'vaga'] });
  }
  ...
  async checkHorarioConflitante(idVaga: number, dataInicio: Date, dataFim: Date): Promise<boolean> {
