...
export class ReservaService {
  ...
  async createReserva(data: any) {
    ...
    const conflito = await this.reservaRepository.checkHorarioConflitante(data.idVaga, data.dataInicio, data.dataFim);
    if (conflito) {
      throw new ForbiddenException('A vaga já está reservada nesse intervalo de tempo.');
    }
  ...
  async updateReserva(id: number, data: any) {
    ...
    if (new Date(reserva.data_fim) < new Date()) {
      throw new ForbiddenException('Não é possível atualizar uma reserva que já expirou.');
    }
  ...
  async deleteReserva(id: number) {
    ...
    if (new Date(reserva.data_fim) < new Date()) {
      throw new ForbiddenException('Não é possível excluir uma reserva que já foi finalizada.');
    }
