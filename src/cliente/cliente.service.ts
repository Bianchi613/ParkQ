...
export class ClienteService {
  ...
  async reservarVaga(id: number, data: any) {
    ...
    if (vaga.status !== 'disponivel') {
      throw new ConflictException(`A vaga com ID ${data.id_vaga} não está disponível para reserva`);
    }
  ...
  async cancelarReserva(id: number, data: any) {
    ...
    if (!reserva) {
      throw new NotFoundException(`Reserva com ID ${data.id_reserva} não encontrada`);
    }
