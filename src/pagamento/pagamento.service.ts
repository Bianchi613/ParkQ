...
export class PagamentoService {
  ...
  async updatePagamento(id: number, data: Partial<Pagamento>): Promise<Pagamento> {
    const [rowsUpdated, [updatedPagamento]] = await this.pagamentoRepository.update(id, data);
    if (rowsUpdated === 0) {
      throw new NotFoundException(`Pagamento with ID ${id} not found.`);
    }
